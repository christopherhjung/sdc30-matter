...
static void air_quality_sensor_notification(uint16_t endpoint_id, float co2_ppm, void *user_data)
{
    ...
    chip::DeviceLayer::SystemLayer().ScheduleLambda([endpoint_id, co2_ppm]() {
        ...
        // CO2 is typically measured in ppm (parts per million)
        // Matter expects the value as a float, so we store it directly
        // If your cluster expects a different format, adjust accordingly
        val.val.f = co2_ppm;
...
extern "C" void app_main()
{
    ...
    endpoint_t * air_quality_sensor_ep = endpoint::air_quality_sensor::create(node, &air_quality_sensor_config, ENDPOINT_FLAG_NONE, nullptr);
    ABORT_APP_ON_FAILURE(air_quality_sensor_ep != nullptr, ESP_LOGE(TAG, "Failed to create temperature_sensor endpoint"));
    ...
    static scd30_sensor_config_t scd30_config = {
        .air_quality = {
            .cb = air_quality_sensor_notification,
    ...
    err = scd30_sensor_init(&scd30_config);
